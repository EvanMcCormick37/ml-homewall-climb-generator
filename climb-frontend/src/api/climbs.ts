...
import { gradeToDifficulty, type GradeScale } from "@/utils/climbs";
...
export async function getClimbs(
  ...
  // Convert grade strings to difficulty floats at the API boundary
  const scale = filters.gradeScale as GradeScale;
  params.append("min_difficulty", gradeToDifficulty(filters.minGrade, scale).toString());
  params.append("max_difficulty", gradeToDifficulty(filters.maxGrade, scale).toString());

  if (filters.includeProjects !== undefined) {
    params.append("include_projects", String(filters.includeProjects));
  }
  if (filters.setterName) params.append("setter_name", filters.setterName);
  if (filters.nameIncludes) params.append("name_includes", filters.nameIncludes);
  if (filters.holdsInclude) {
    filters.holdsInclude.forEach((h) => params.append("holds_include", h.toString()));
  }
...
export async function createClimb(
  ...
  if (data.holdset.start.length === 0) throw new Error("At least one start hold is required");
  if (data.holdset.start.length > 2) throw new Error("Maximum of 2 start holds allowed");
  if (data.holdset.finish.length === 0) throw new Error("At least one finish hold is required");
  if (data.holdset.finish.length > 2) throw new Error("Maximum of 2 finish holds allowed");
